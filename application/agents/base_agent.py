...
class BaseAgent(ABC):
    ...
    GENERAL_QUERY_KEYWORDS = [
        ...
        'cómo funciona', 'para qué sirve', 'información general', 'vista general'
    ]
    ...
    def _is_general_query(self, query: str) -> bool:
        ...
        # Si contiene palabras clave de archivo específico, no es general
        file_keywords = ['archivo', 'función', 'clase', 'método', 'código de']
        if any(kw in query_lower for kw in file_keywords):
            if self._extract_file_name(query):
                return False
    ...
    def _infer_file_purpose(self, filename: str, language: str) -> str:
        """
        Infiere el propósito del archivo por su nombre.
        """
    ...
    def _extract_file_summary(self, file_path: Path, content: str, language: str) -> Dict[str, Any]:
        ...
        Solo extrae nombres, no fragmentos de código.
        """
        summary = {
            'name': file_path.name,
        ...
        # HTML: extraer título y estructura
        if language == 'HTML':
        ...
        # CSS: extraer variables y clases
        elif language == 'CSS':
        ...
        # JavaScript: extraer funciones, clases, variables (solo nombres)
        elif language == 'JavaScript':
            # Funciones
        ...
        # Python: extraer funciones y clases (solo nombres)
        elif language == 'Python':
    ...
    def _build_compact_context(self, repo_summary: Dict[str, Any]) -> str:
        """
        Construye un contexto compacto sin fragmentos de código.
        """
        context_parts = []
        ...
        for file in repo_summary['files'][:8]:
            ...
            if file.get('variables'):
                file_info += f"\n  Variables: {', '.join(file['variables'][:3])}"
